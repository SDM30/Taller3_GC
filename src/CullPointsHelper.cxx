...
/**
 * @brief Constructor: Copia los puntos de la triangulación original a la
 * reducida y crea el mapeo entre puntos e índices para referencia futura
 */
CullPointsHelper::CullPointsHelper(double gamma, uint delta,
                                   const TDelaunay &original_tri)
    : gamma(gamma), delta(delta), original_tri(original_tri), reduced_tri() {
  size_t idx = 0;
  ...
  std::cout << "CullPointsHelper inicializado con " << orgin_points.size()
            << " puntos" << std::endl;
}
...
std::optional<size_t> CullPointsHelper::getIndex(const TPoint &point) const {
  auto it =
      std::find_if(orgin_points.begin(), orgin_points.end(),
                   [&point](const VertIndx &vi) { return vi.point == point; });
...
std::vector<CullPointsHelper::TPoint> CullPointsHelper::neigh(TPoint vertex) {
  ...
  std::vector<bool> m(original_tri.number_of_vertices(),
                      false); // Marcas de visitados
  std::queue<TPoint> q;       // Cola para BFS
  auto first = getIndex(vertex);
  if (first == std::nullopt)
    return neighbors; // Vértice no encontrado
  ...
  while (!q.empty()) {
    ...
    if (vertex_handle == Vertex_handle() ||
        reduced_tri.is_infinite(vertex_handle))
      return neighbors;
    ...
    for (auto nIt = local_neighbors.begin(); nIt != local_neighbors.end();
         ++nIt) {
...
void CullPointsHelper::reduceOverThreshold() {
  ...
  for (auto vIt = original_tri.finite_vertices_begin();
    ...
    if (!vNotEmptyOrInf(vertex_handle, false))
      continue;
    ...
    std::cout << "v=(" << p.x() << "," << p.y() << ")"
              << " vecinos=" << v_delta.size() << " avg_z=" << avg_z
              << " sigma_z=" << sigma_z << " threshold=" << threshold
              << std::endl;

    // Paso 4: Marcar puntos que exceden el umbral
    for (const auto &v : v_delta) {
      auto vertex = reduced_tri.nearest_vertex(v);
      if (!vNotEmptyOrInf(vertex, false))
        continue;
      TReal diff = std::abs(vertex->info() - avg_z);

      std::cout << "  candidato info=" << vertex->info() << " diff=" << diff
  ...
  std::cout << "Puntos restantes: " << reduced_tri.number_of_vertices()
            << std::endl;
}
...
CullPointsHelper::getReducedTriangulation() const {
  return reduced_tri;
}
